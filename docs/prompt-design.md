...
|---|---|---|
| `--judge` | 같은 응답 원문을 다시 판정 → 판정 노이즈만 | 온도 고정 전 σ = **12.9%p** → 고정 후 **0.0%p** |
| `--collect` | 같은 질문을 다시 수집·판정 → 합성 노이즈 | 기준선 σ = 19.6%p |

처음 재보니 판정 노이즈가 12.9%p였다(20셀 중 1셀에서 언급 판정이 뒤집혔다). 온도가 코드에
없어 SDK 기본값(비결정적)을 쓰고 있었기 때문이다. **판정 온도를 0으로 고정하니 0.0%p가
됐다** — 같은 조건(언급률 60%)에서 60회 판정, 뒤집힘 0건. 판정은 측정 도구이므로 같은
입력에 같은 답이 나와야 한다(`server/engines/judgeSampling.ts`).

남은 노이즈는 전부 수집 쪽이다. 수집 호출에는 `googleSearch` 그라운딩이 붙으므로 이 부분은
모델 샘플링만이 아니라 **검색이 가져오는 문서가 매번 달라지는 것**까지 포함한다. 그건 계측기의
흔들림이 아니라 실제 노출의 변동이므로 고정하지 않는다 — 수집 온도를 우리가 바꾸면 그건
더 이상 "실제 사용자가 물었을 때 나오는 답"이 아니다.

**판단 엔진을 Claude로 두면 이 고정이 불가능하다.** 현행 Claude 모델(Opus 5·4.8·4.7,
Sonnet 5, Fable 5 계열)은 `temperature`·`top_p`·`top_k`를 400으로 거부한다(사고 깊이를
`output_config.effort`로 조절하는 설계로 바뀌었다). 기본 판단 엔진을 Gemini로 유지하는
이유가 하나 더 늘었다.
