...
### 2-1. 반복 3회 → 2회, 문항 12 → 18 (v0.1.50, 질문 은행 v2)
...
**반복을 1회로 없애지 않은 이유**는 반복이 정밀도 장치이기만 한 게 아니라는 데 있다.
반복은 이 원칙이 말하는 비결정성 자체를 재는 장치이고, 실측값은 **표준편차 19.6%p** —
다섯 번에 한 번은 판정이 뒤집힌다. 질문 승패·질문별 감성 화면도 질문당 관측이 2개는 있어야
한 응답의 운이 결론이 되지 않는다.

**주의 — 이 변경은 척도를 갈아치운다.** 질문이 달라지면 언급률·SoM의 모집단이 바뀌어 같은
브랜드라도 값이 움직인다. 그래서 질문 은행 버전을 `v1`(12문항) → `v2`(18문항)로 올려 옛
집합을 보존하고, 스코어카드에 `questionBankVersion`을 기록한다. v2 이전 주차와의 점수 비교는
"같은 척도로 잰 값"이 아니다.
...
적용한 18문항 × 2회**다(`docs/architecture-option-c.md` 참고). Gemini 단독이면 브랜드당 수집
36콜 + 판정 약 95콜이다.

분석 단계는 반복분의 평균 텍스트가 아니라 **각 반복을 독립적으로 판정**한 뒤 그 판정 결과들을
평균·CI 처리해야 한다는 점에 주의. (하나로 합쳐 요약한 뒤 분석하면 반복 측정의 통계적 의미가
사라진다.)
