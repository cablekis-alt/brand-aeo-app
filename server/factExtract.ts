...
/**
 * 값의 길이 상한 — **판별자가 아니라 폭주 방지 안전망이다.**
 *
 * 처음에 40자로 뒀다가 실측에서 정상 주소를 잘라 먹었다:
 *   "경기도 성남시 분당구 성남대로 884-3 가온미디어빌딩 (야탑동 513-4)"  42자
 * 길이로는 갈리지 않는다. 같은 실측에서 문장은 19자("건물과 객실 안은 절대 금연입니다.")
 * 짜리도 50자짜리도 있었다. 실제로 문장을 잡는 것은 아래 종결어미 규칙이고, 이 상한은
 * 어미도 마침표도 없이 한없이 긴 값만 막는다.
 */
